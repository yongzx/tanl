...
from custom_models.bart_adapter_utils import label_smoothed_nll_loss
...
class EncoderLayerWithAdapter(EncoderLayer):
    ...
    def __init__(self, config: BartConfig):
        ...
        self.adapter_down_weight = torch.zeros(self.embed_dim, self.adapter_dim).to(self.device)
        self.adapter_down_bias = torch.zeros(self.adapter_dim).to(self.device)

        self.adapter_up_weight = torch.zeros(self.adapter_dim, self.embed_dim).to(self.device)
        self.adapter_up_bias = torch.zeros(self.embed_dim).to(self.device)
...
class MyBartWithAdapter(BartForConditionalGenerationWithAdapter):
    def forward(self, input_ids, attention_mask=None, encoder_outputs=None,
            ...
            use_cache=False, is_training=True, labels=None, **kwargs):
